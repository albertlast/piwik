 ...
 * @package    Zend_Db
 * @subpackage Adapter
 * @copyright  Jenei Viktor Attila // Fixed By Gencer Gen�
...
class Zend_Db_Adapter_Pgsql extends Zend_Db_Adapter_Abstract {
	...
	if(strpos($sql, 'HOW COLUMNS FROM')){
	    $sql = str_replace('SHOW COLUMNS FROM','SELECT column_name FROM information_schema.columns WHERE table_name =',$sql);
	    $sql = str_replace('"',"'",$sql);
	}
	
	if(strpos($sql, 'ON DUPLICATE KEY')){
	    //ON CONFLICT ON CONSTRAINT distributors_pkey DO UPDATE SET dname = EXCLUDED.dname;
	    preg_match('/INTO\w*\W*(\w*)/i',$sql, $table_name);
	    $sql = str_replace('ON DUPLICATE KEY UPDATE', 'ON CONFLICT ON CONSTRAINT '.$table_name[1].'_pkey DO UPDATE SET',$sql);
	}
	...
	$pg_version = pg_version($this->_connection);
	// if we got a Beta Version
	if (stripos($pg_version[server], 'beta') !== false)
		$pg_version[server] = substr($pg_version[server], 0, stripos($pg_version[server], 'beta')).'.0';
	// or RC
	if (stripos($pg_version[server], 'rc') !== false)
		$pg_version[server] = substr($pg_version[server], 0, stripos($pg_version[server], 'rc')).'.0';
